...
exports.updateChallengeLikeCount = (change, _context) => {
  const challengeId = change.after.exists ? change.after.data().challengeId : change.before.data().challengeId
  return getNumberOfChallengeLikes(challengeId)
    .then(count => setChallengeLikeCount(challengeId, count) )
}
